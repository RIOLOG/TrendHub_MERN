...
const Homapage = () => {
  ...
  const getAllProducts = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`http://localhost:8081/api/v1/product/product-list/${page}`);
      setLoading(false);
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get("http://localhost:8081/api/v1/product/product-count");
      setTotal(data?.total);
    } 
    catch (error) 
    {
